    ...
    if ($result->num_rows == 1) {
        ...
        if (password_verify($contrasena, $row['contrasena'])) {
            ...
            if ($row['tipo'] == 'candidato') {
                header("Location: /src/views/perfilcandidato.html");
            } else {
                header("Location: /src/views/perfilempresa.html");
            }
            exit();
        } else {
...
if (isset($_GET['Salir'])) {
    ...
    session_unset();
    session_destroy();
    header("Location: /src/views/ingreso.html", true, 302); // Redirección permanente (302)
exit();
}
